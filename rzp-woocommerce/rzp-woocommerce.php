	...
	public function admin_notice() {
		...
		if ( get_transient( 'rzpwc-admin-notice-on-activation' ) ) {
			...
			<div class="notice notice-success">
				...
				printf( wp_kses_post( __( 'Thanks for installing %1$s v%2$s plugin. Click <a href="%3$s">here</a> to configure plugin settings.', 'rzp-woocommerce' ) ), 'Razorpay Payment Links for WooCommerce', esc_html( RZPWC_VERSION ), esc_url( admin_url( 'admin.php?page=wc-settings&tab=checkout&section=wc-razorpay' ) ) );
				?>
				</strong></p>
			</div> 
			<?php
